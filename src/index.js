...
function compareDatesToday(todoDate) {
  const result = isEqual(set(new Date(), {
    hours: 0, minutes: 0, seconds: 0, milliseconds: 0,
  }), set(new Date(todoDate), {
    hours: 0, minutes: 0, seconds: 0, milliseconds: 0,
  }));
  return result;
}
...
function compareDatesWeek(todoDate) {
  if (isAfter(parseISO(todoDate), new Date())
  && isBefore(parseISO(todoDate), add(new Date(), { days: 7 }))) {
    return true;
  }
}
...
function deleteTodo(deleteBtn) {
  ...
  localStorage.setItem("todoList", JSON.stringify(storageTodoList));
  if (todayList) {
    const todayTodo = todayList.find((data) => data.title === deleteBtn.parentNode.id);
    todayList.splice(todayList.indexOf(todayTodo), 1);
  }
  if (weekList) {
    const weekTodo = weekList.find((data) => data.title === deleteBtn.parentNode.id);
